...
void CGame::DrawDialogBox_GuideMap(short msX, short msY, char cLB)
{
	...
	if (m_bZoomMap)
	{
	 ...
	 // Slime
			...
			else if (m_cMapIndex == 7) { // Default
			// Slime
				if (101 >= shX && 101 <= shX + 128 && 87 >= shY && 87 <= shY + 128)
					m_pSprite[DEF_SPRID_INTERFACE_MONSTER]->PutSpriteFast(sX + 101 - shX, sY + 87 - shY, 10, m_dwCurTime);
	...
	else // Sans zoom
	{
		...
		if (m_bNpcMap) {//MAPA CHICO
			if (m_cMapIndex == 11) { // Aresden
				...
				m_pSprite[DEF_SPRID_INTERFACE_MONSTER]->PutSpriteFast(sX + ((101 * 128) / m_pMapData->m_sMapSizeX), sY + ((87 * 128) / m_pMapData->m_sMapSizeX), 10, m_dwCurTime);
